...
export class EnvironmentController {
    ...
    public async loadEnvironment(
        ...
        const existingEntity: MVEnvironmentEntity =
            this._entityService.getEnvironmentEntity(entityConfig.id);
        if (existingEntity) {
            if (
                existingEntity !== this._entityService.activeEnvironmentEntity
            ) {
                this._entityService.dispose(
        ...
        let entity: MVEnvironmentEntity =
            await this._entityService.addEnvironment(
                entityConfig,
                entityConfig.id,
        ...
        const updateConfigurationPromise =
            this._configurationService.updateConfiguration(entity, []);

        if (preloadOnly) {
            await loadRigPromise;
            await updateConfigurationPromise;
            this._configurationService.disableAllLayers(entity);
            await this._entityService.lazyLoadRemainingLayersAndTextures(
                entity
            );
            return entity;
        }
        ...
        // Update Lights
        const updateLightsAndSceneSettingsPromise =
            this._entityService.updateLightsAndSceneSettings(
                tmpCameraCategory,
                entity
            );

        await Promise.all([updateConfigurationPromise, loadRigPromise]);
        ...
        this._eventDispatcherService.publish(MVEventTypes.onEnvironmentLoaded, {
            entity
        });
    ...
    public async updateConfiguration(
        ...
        const entity: MVEnvironmentEntity =
            this._entityService.getEnvironmentEntity(id);
        ...
        await this._configurationService.updateConfiguration(
            entity,
            configurationCodes
        );
        ...
        const currentCameraShot: MVCameraShot =
