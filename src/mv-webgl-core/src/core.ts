...
import { createContainer } from './ioc/inversify.config';
...
export class Core {
    ...
    private readonly _ioc: Container = createContainer();
    ...
    constructor(
        ...
        settings: CoreSettingsObject
    ) {
        if (!containerElement) {
            MVLogger.fatal(
                CoreError.InvalidParameterError,
                'No html container id or element provided in Core constructor.'
            );
        }
        ...
        if (typeof containerElement === 'string') {
            ...
            if (!htmlContainer) {
                MVLogger.fatal(
                    CoreError.NotFound,
                    `No html element with the provided id (${containerElement}) found`
                );
            }
        } else {
        ...
        this._ioc.bind<Scene>(TYPES.Scene).toConstantValue(this._scene);
        this._ioc
            .bind<CoreWraperElement>(TYPES.CoreWraperElement)
            .toConstantValue(this._container);
        this._ioc
            .bind<CoreCanvasElement>(TYPES.Canvas)
            .toConstantValue(this._canvas);
        this._ioc
            .bind<ResolverLocator>(TYPES.ResolverLocator)
            .toConstantValue(resolverLocator);
        this._ioc
            .bind<CoreSettings>(TYPES.CoreSettings)
            .toConstantValue(new CoreSettings(settings, this._scene));
        this._ioc.load(services, controlers);
        this._sceneOptimizer = this._ioc.get<MVSceneOptimizerService>(
            TYPES.MVSceneOptimizerService
        );
        this._sceneSettingsService = this._ioc.get<SceneSettingsService>(
            TYPES.SceneSettingsService
        );
        this._eventDispatcherService = this._ioc.get<EventDispatcherService>(
            TYPES.EventDispatcherService
        );
        this._settings = this._ioc.get<CoreSettings>(TYPES.CoreSettings);
        this._entityService = this._ioc.get<EntityService>(TYPES.EntityService);
        this._actionItemService = this._ioc.get<ActionItemService>(
            TYPES.ActionItemService
        );
        this._cameraService = this._ioc.get<CameraService>(TYPES.CameraService);
        this.Camera = this._ioc.get<CameraController>(TYPES.CameraController);
        this.Product = this._ioc.get<ProductController>(
            TYPES.ProductController
        );
        this.Environment = this._ioc.get<EnvironmentController>(
            TYPES.EnvironmentController
        );
        this.Animation = this._ioc.get<AnimationController>(
            TYPES.AnimationController
        );
        this.Controls = this._ioc.get<ControlsController>(
            TYPES.ControlsController
        );
        this.Light = this._ioc.get<LightController>(TYPES.LightController);
        this.ActionItem = this._ioc.get<ActionItemController>(
            TYPES.ActionItemController
        );
        this.Scene = this._ioc.get<SceneController>(TYPES.SceneController);
        const platform = this._ioc.get<PlatformService>(TYPES.PlatformService);
        ...
        this.renderSubscription$ = this.render$
            ...
            .subscribe({
                error: (err: Error) => {
                    MVLogger.error(
                        `There has been a problem with render stream: ${err}`
                    );
                }
            });
    ...
    private registerEventListeners(): void {
        ...
        const debouncedResizeEvent = resizeEvent.pipe(
            debounceTime(100),
            takeUntil(onDestroy$)
        );
        debouncedResizeEvent.subscribe(() => {
            ...
            this.resize();
            // Update camera
            const cs = this._ioc.get<CameraService>(TYPES.CameraService);
    ...
    private clickEventListener() {
        ...
        if (
            !this._lastClickEventTime ||
            currentTimeInMS - this._lastClickEventTime < 500
        ) {
            ...
            if (this._multiClickEventCounter >= 20) {
                ...
                const clicksUntilStatsToggle =
                    20 - this._multiClickEventCounter;
                MVLogger.debug(
                    clicksUntilStatsToggle +
                        ' more clicks until stats are toggled.'
                );
            }
        } else {
    ...
    public toggleInspector(): void {
        this._inspectorState
            ...
            : this._scene.debugLayer.show({ enablePopup: false });
    ...
    public async updateMaterialMapping(
        ...
        return this._entityService.updateMaterialMapping(
            entity,
            updatedMaterialMappingsJson,
