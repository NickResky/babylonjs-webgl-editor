...
import { FileAccessService, FileType } from './services/file-access/file-access.service';
...
export class App {
  ...
  private async loadEntityConfig() {
    ...
    if (store.entityBasePath) {
      ...
      this.dataService.projectSettings = {
        ...
        baseProjectUrl: 'file://' + store.entityBasePath.replace(/\\/g, '/'), // necessary for windows file system
  ...
  public async openEntityFileSelector(productionMode?: boolean): Promise<void> {
    const openDialogOptions = {
      title: 'Open an entitiy config file in JSON Format',
      filters: [{ name: 'JSON', extensions: ['json'] }],
    };
    const result = await (window as any).electronAPI.showOpenDialogSync(openDialogOptions);
    console.log(result);
    const entityConfig = await this.fileService.getFile(
  ...
  resizeSidebar(): void {
    this.sidebarWidthIndex =
      ++this.sidebarWidthIndex >= this.sidebarWidths.length ? 0 : this.sidebarWidthIndex;
