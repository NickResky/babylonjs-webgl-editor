...
export class EnvironmentService {
  ...
  private _selectedEnvironmentConfigurationCode$ = new BehaviorSubject<string>(null);
  public selectedEnvironmentConfigurationCode$ =
    this._selectedEnvironmentConfigurationCode$.asObservable();
  private _selectedEnvironmentEntity$ = new BehaviorSubject<MVEnvironmentEntity>(null);
  private _selectedEnvironmentEntity: MVEnvironmentEntity;
  public selectedEnvironmentEntity$ = this._selectedEnvironmentEntity$.asObservable();
  ...
  constructor(
    ...
    this.dataService.projectSettings$.subscribe((projectSettings: ProjectSettings) => {
      this._baseURL = projectSettings.baseProjectUrl;
    });

    this.cameraService.activeCameraShotCategory$.subscribe((activeCameraShotCategory: string) => {
  ...
  public async onEnvironmentSavedClicked(
    ...
    const environmentSetting: MVEnvironmentConfig = new Mappers.ENVIRONMENT(
      scene,
      backgroundImageName && backgroundImageName.length > 0 ? backgroundImageName : null
    ).toJSON();
    const environmentSettings: MVEnvironmentConfigs = await this.updateEnvironmentFile(
      basePath,
      environmentSetting
    );
  ...
  private async updateEnvironmentEntityWithLights(scene) {
    ...
    const environmentEntityConfig = this._selectedEnvironmentEntity.entityConfig;

    if (!environmentEntityConfig.lights) {
      environmentEntityConfig.lights = {};
    }

    environmentEntityConfig.lights[this._selectedEnvironmentConfigurationCode] = [];

    lights.map((light) => {
      environmentEntityConfig.lights[this._selectedEnvironmentConfigurationCode].push(light);
    });
  ...
  public async updateEnvironmentFile(
    ...
    const environenmentConfigUrl =
      this._selectedEnvironmentEntity.entityConfig.environmentConfigRelative;
    const fileName = environenmentConfigUrl.slice(environenmentConfigUrl.lastIndexOf('/') + 1);
    const newPath = basePath.replace('file://', '') + environenmentConfigUrl.replace(fileName, '');

    const environmentConfigs: MVEnvironmentConfigs = JSON.parse(
      await this.fileService.getFile(newPath, fileName.replace('.json', ''), FileType.JSON)
    );
    environmentConfigs[this._selectedEnvironmentConfigurationCode] = environment;
  ...
  public selectEnvironmentConfig(environmentConfigCode: string) {
    ...
    this._selectedEnvironmentConfigurationCode$.next(this._selectedEnvironmentConfigurationCode);
  }

  public async updateEnvironmentConfig(): Promise<void> {
    await this._core.Environment.updateConfiguration(this._selectedEnvironmentEntity.uuid, [
  ...
  public async loadDefaultEnvironment(
    ...
    if (environmentUrls && environmentUrls.length > 0) {
      return this.selectEnvironment(environmentUrls[0]);
    }
  }

  public async selectEnvironment(environmentUrl: string): Promise<MVEnvironmentEntity> {
    this.dataService.setLoading(true);
    await this._core.stopRender();
    await this.removeLightsFromScene();
    const previousEnvironmentEntityConfig = this._selectedEnvironmentEntity?.entityConfig;
    const environmentCategory = this._selectedEnvironmentConfigurationCode
      ? this._selectedEnvironmentConfigurationCode
      : this.cameraService.activeCameraShotCategory;

    const environmentEntityConfig: MVEntityConfig = (window as any).electronAPI.fsReadJSONSync(
      this._baseURL.replace('file://', '') + environmentUrl
    );

    await this.entityService.updateGlbFileRegistryJSON(environmentEntityConfig);

    console.log(`Combining JSON files of entity ${environmentUrl.replace('file://', '')} ...`);
    ...
    const combinedEntityConfigString = 'data:' + JSON.stringify(combinedEntityConfig);

    console.log(`Loading entity ${environmentUrl.replace('file://', '')} ...`);
  ...
  public async refreshEnvironment(environmentSettings: MVEnvironmentConfigs): Promise<void> {
    this._selectedEnvironmentEntity.environmentSceneSettings = environmentSettings;
    await this.removeLightsFromScene();
    await this._core.Environment.updateConfiguration(this._selectedEnvironmentEntity.uuid, [
      this._selectedEnvironmentConfigurationCode,
    ]);
  ...
  public async updateEnvironmentTexture(filePath: string, fileName: string): Promise<void> {
    const newEnviromentTexture = new CubeTexture(filePath + fileName, this._core.getScene());
  ...
  public async setEnvironmentEntityConfig(relativeUrl): Promise<MVEntityConfig> {
    const baseUrl = this._baseURL.replace('file://', '');
    this._productEntityConfig = await (window as any).electronAPI.fsReadJSONSync(
      baseUrl + relativeUrl
    );
   ...
   */
  public async isMeshFilePartOfSelectedEnvironment(meshFileName: string): Promise<boolean> {
    const fullPath =
      this._entityBaseUrl.replace('file://', '') +
      this._selectedEnvironmentEntity.entityConfig.meshesUrlRelative;
    const filesInMeshDirectory: string[] = await this.fileService.getFilesInDirectory(fullPath);
