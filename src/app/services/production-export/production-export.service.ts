...
import { ConversionTarget, ConverterService } from '../converter/converter.service';
...
export class ProductionExportService {
  ...
  selectEntityFiles() {
    ...
    const paths = (window as any).electronAPI(openDialogOptions);
  ...
  setEntities(paths: string[]) {
    ...
    paths.forEach((path) => {
      path = path.replace(/\\/g, '/');
      const root = (window as any).electronAPI.path().dirname(path).replace(/\\/g, '/') + '/';
      ...
      try {
        const entityConfig = (window as any).electronAPI.fsReadJSONSync(path);
  ...
  async startBuild(convertTextures: boolean): Promise<boolean> {
    ...
    this._assetsBaseUrl =
      (window as any).electronAPI.path().dirname(this._entitiesBuildMetaData[0].path) + '/';
    ...
    for (let entityBuildMetaData of this._entitiesBuildMetaData) {
      ...
      try {
        ...
        this._entities$.next(this._entitiesBuildMetaData);
        let combinedFile = await this.combineJsonFilesOfEntity(entityBuildMetaData);
        ...
        combinedFile = await this.exportProductionMeshes(entityBuildMetaData, combinedFile);
    ...
    const coreProductionSettingPath = this._assetsBaseUrl + coreProductionSettingsFileName;
    if ((window as any).electronAPI.fsExistsSync(coreProductionSettingPath)) {
   ...
   */
  async exportProductionMeshes(entity: EntityBuildMetaData, combinedJsonFile: any): Promise<any> {
    const fbxFilesDirectoryAbsolute = entity.entityConfig['fbxFilesDirectoryAbsolute'];
    // if (fbxFilesDirectoryAbsolute && fbxFilesDirectoryAbsolute.length > 0) {

    const entityConfigFileName = (window as any).electronAPI.path().basename(entity.path);
    ...
    const outputMeshSettingsUrl = this._exportDir + entity.entityConfig.meshSettingsRelative;
    const glbMetaDataUrl =
      this._assetsBaseUrl + entity.entityConfig.meshesUrlRelative + '/glbFileMetaData.json';
    ...
    const conversionStatus = await this.converterService.runGlbConversion({
      ...
      preventDracoCompressionDuringBuild: entity.entityConfig.preventDracoCompressionDuringBuild,
      preventVertexColorDeletionDuringBuild:
        entity.entityConfig.preventVertexColorDeletionDuringBuild,
    });

    combinedJsonFile['productionMeshSettings'] = conversionStatus.meshSettings;
  ...
  async copyRemainingGlbFiles(entity: EntityBuildMetaData) {
    ...
    const glbFileNames = (window as any).electronAPI
      .fsReaddirSync(this._assetsBaseUrl + entity.entityConfig.meshesUrlRelative)
    ...
    glbFileNames.forEach((glbFileName) => {
      ...
      if (!fileExistsInFbxDirectory) {
        const fullFilePath =
          this._assetsBaseUrl + entity.entityConfig.meshesUrlRelative + '/' + glbFileName + '.glb';
        let fullOutputFilePath =
          this._exportDir + entity.entityConfig.meshesUrlRelative + '/' + glbFileName + '.glb';
        const fileExistsInOutputDir = (window as any).electronAPI.fsExistsSync(fullOutputFilePath);
        ...
        (window as any).electronAPI.fsCopyFileSync(fullFilePath, fullOutputFilePath);
  ...
  async exportProductionTextures(entityBuildMetaData: EntityBuildMetaData) {
    ...
    const texturesUrlRelative = entityBuildMetaData.entityConfig.texturesUrlRelative;
    ...
    const mobileTexturesUrlRelative = entityBuildMetaData.entityConfig.mobileTexturesUrlRelative;
  ...
  updateGlbFileRegistry(entity: EntityBuildMetaData) {
    ...
    const allFileNames = (window as any).electronAPI.fsReaddirSync(meshesUrl).toString();
    ...
    (window as any).electronAPI.fsWriteFileSync(glbRegistryPath, allFileNames);
    this.fileAccessService.setReadAndWritePermissions(glbRegistryPath);
  }

  async writeExportFile(entity: EntityBuildMetaData, combinedEntityConfig: any) {
  ...
  public async combineCameraShots(
    ...
    if (cameraShotsMetaDataCopy) {
      for (let cameraShotId of Object.keys(cameraShotsMetaDataCopy)) {
        const cameraShotMetaData: MVCameraShotMetaData = cameraShotsMetaDataCopy[cameraShotId];
        ...
        if (cameraShotMetaData.urlRelative) {
          ...
          try {
            const cameraSettings = (window as any).electronAPI.fsReadJSONSync(url);
            cameraShotMetaData['cameraShotSettings'] = cameraSettings;
          } catch (error) {
        ...
        if (cameraShotMetaData.mobileUrlRelative) {
          const url = entityBaseUrl + cameraShotMetaData.mobileUrlRelative;
          try {
            const cameraSettings = (window as any).electronAPI.fsReadJSONSync(url);
            cameraShotMetaData['cameraShotSettingsMobile'] = cameraSettings;
          } catch (error) {
            const errorMessage = `Camera shot JSON ${url} not found for camera shot ${cameraShotId}`;
            console.error(errorMessage);
            this.notifier.notify('error', errorMessage);
  ...
  public async combineJsonFilesOfEntity(entityBuildMetaData: EntityBuildMetaData) {
    ...
    if (entityConfig.cameraShots) {
      entityConfig.cameraShots = await this.combineCameraShots(entityConfig.cameraShots, root);
    }
    ...
    if (!!meshSettingsRelative) {
      const meshSettings = (window as any).electronAPI.fsReadJSONSync(root + meshSettingsRelative);
      // delete entity.meshSettingsRelative;
      entityConfig.meshSettings = meshSettings;
    }
    const productionMeshSettingsRelative = entityConfig.productionMeshSettingsRelative;
    if (!!productionMeshSettingsRelative) {
      const productionMeshSettings = (window as any).electronAPI.fsReadJSONSync(
        root + productionMeshSettingsRelative
      );
      // delete entity.productionMeshSettingsRelative;
      entityConfig.productionMeshSettings = productionMeshSettings;
    }
    const ruleEngineConfigUrlRelative = entityConfig.ruleEngineConfigUrlRelative;
    if (!!ruleEngineConfigUrlRelative) {
      if ((window as any).electronAPI.fsExistsSync(root + ruleEngineConfigUrlRelative)) {
        const ruleEngineConfig = (window as any).electronAPI.fsReadJSONSync(
    ...
    const materialMappingsUrlRelative = entityConfig.materialMappingsUrlRelative;
    if (!!materialMappingsUrlRelative) {
      ...
      materialMappings.materialAllocators = materialMappings.materialAllocators.map((p) => {
        const materialPath = `${entityConfig.materialsUrlRelative}${p.mapping}`;

        let material = entityConfig.materials[materialPath];
        if (!material && (window as any).electronAPI.fsExistsSync(root + materialPath)) {
          material = (window as any).electronAPI.fsReadJSONSync(root + materialPath);
      ...
      materialMappings.switchMaterials = materialMappings.switchMaterials.map((p) => {
      ...
      if (entityBuildMetaData.entityConfig.defaultNodeMaterialUrlRelative) {
        const materialPath =
          entityConfig.materialsUrlRelative +
          entityBuildMetaData.entityConfig?.defaultNodeMaterialUrlRelative;
        try {
          const defaultNodeMaterial = (window as any).electronAPI.fsReadJSONSync(
            root + materialPath
          );
    ...
    const lightmapTexturesUrlRelative = entityConfig.lightmapTexturesUrlRelative;
    if (!!lightmapTexturesUrlRelative) {
      ...
      entityConfig.lightmapTextures = lightmapTextures && lightmapTextures.files;
      ...
      if (!!mobileLightmapTexturesUrlRelative) {
        entityConfig.mobileLightmapTextures = lightmapTextures && lightmapTextures.files;
      }
    }
    // const mobileLightmapTexturesUrlRelative = entityConfig.mobileLightmapTexturesUrlRelative;
    // if (!!mobileLightmapTexturesUrlRelative) {
    ...
    const environmentConfigRelative = entityConfig.environmentConfigRelative;
    if (!!environmentConfigRelative) {
      const environmentConfig = (window as any).electronAPI.fsReadJSONSync(
        root + environmentConfigRelative
      );
  ...
  private copyActionItems(entityBuildMetaData: EntityBuildMetaData) {
    ...
    const actionItemUrlExport = this._exportDir + entityConfig.actionItemUrlRelative;
    ...
    if (
      ...
      (window as any).electronAPI.fsCopySync(actionItemUrl, actionItemUrlExport);

      const actionItemOptions: ActionItemsOptionsJSON = (window as any).electronAPI.fsReadJSONSync(
        actionItemUrl
      );
      ...
      const defaultMaterialUrl =
        root + actionItemOptions.materialsUrlRelative + actionItemOptions.defaultMaterial;
      const defaultMaterialUrlExport =
        this._exportDir +
        actionItemOptions.materialsUrlRelative +
        actionItemOptions.defaultMaterial;

      const animationsUrl = root + actionItemOptions.animationsUrlRelative;
      const animationsUrlExport = this._exportDir + actionItemOptions.animationsUrlRelative;

      if (
        actionItemOptions.defaultMaterial &&
        !(window as any).electronAPI.fsExistsSync(defaultMaterialUrlExport)
      ) {
        (window as any).electronAPI.fsCopySync(defaultMaterialUrl, defaultMaterialUrlExport);
      }

      const actionItemTexturesUrl = root + actionItemOptions.texturesUrlRelative;
      const actionItemTexturesUrlExport = this._exportDir + actionItemOptions.texturesUrlRelative;
      if (
        actionItemOptions.texturesUrlRelative &&
        !(window as any).electronAPI.fsExistsSync(actionItemTexturesUrlExport)
      ) {
        (window as any).electronAPI.fsCopySync(actionItemTexturesUrl, actionItemTexturesUrlExport, {
          recursive: true,
        });
      }

      actionItemOptions.actionItems.forEach((actionItemOption: ActionItemOptionsJSON) => {
        const materialUrl =
          root + actionItemOptions.materialsUrlRelative + actionItemOption.material;
        const materialUrlExport =
          this._exportDir + actionItemOptions.materialsUrlRelative + actionItemOption.material;

        if (
          actionItemOption.material &&
          !(window as any).electronAPI.fsExistsSync(materialUrlExport)
        ) {
          (window as any).electronAPI.fsCopySync(materialUrl, materialUrlExport);
        }
