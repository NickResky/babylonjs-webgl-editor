...
export class ConverterService {
  ...
  constructor(
    ...
    this.userService.getUserStore().then((userStore) => {
      ...
      if (this.userStore.blenderPath) {
        const blenderPathExists = (window as any).electronAPI.fsExistsSync(
          this.userStore.blenderPath
        );
      ...
      if (this.userStore.nodePath) {
        const nodePathExists = (window as any).electronAPI.fsExistsSync(this.userStore.nodePath);
        if (nodePathExists) {
  ...
  async chooseInputDirectory() {
    ...
    const paths = await (window as any).electronAPI.showOpenDialogSync(openDialogOptions);
  ...
  async chooseBlenderPath() {
    ...
    let path = await (window as any).electronAPI.showOpenDialogSync(openDialogOptions)[0];
    if (path) {
      path = path.replace(/\\/g, '/');
  ...
  async setBlenderConfig(options: ConversionConfigJSON): Promise<ConversionConfigJSON> {
    const basepath = await this.getAppBasePath();
    const configPath = basepath + '/mv-webgl-pipeline-tools/config.json';

    const config: ConversionConfigJSON = (window as any).electronAPI.fsReadJSONSync(configPath);
    ...
    config['blenderToolsPath'] = basepath + '/mv-webgl-pipeline-tools/blender_tools/';
    ...
    config['preventVertexColorDeletionDuringBuild'] = options.preventVertexColorDeletionDuringBuild;
    config['preventDracoCompressionDuringBuild'] = options.preventDracoCompressionDuringBuild;

    this.fileAccessService.setReadAndWritePermissions(configPath);
    (window as any).electronAPI.writeJSONSync(configPath, config, {
      spaces: 2,
    });
  ...
  async runGlbConversion(options: ConversionConfigJSON): Promise<ConversionStatus> {
    this._conversionInProgress$.next(true);

    const config: ConversionConfigJSON = await this.setBlenderConfig(options);
    ...
    try {
      const fileGroupsJson = (window as any).electronAPI.fsReadJson(
        config.tmpPath + '/file_groups.json'
      );
    ...
    if (config.outputMeshSettingsUrl) {
      try {
        meshSettings = await (window as any).electronAPI.fsReadJson(config.outputMeshSettingsUrl);
        if (options.glbMetaDataUrl) {
          glbMetaData = await (window as any).electronAPI.fsReadJson(options.glbMetaDataUrl);
        }
      } catch (error) {
  ...
  async runTextureConversion(
    ...
    const textureConversionSettingsPath = inputDirectory + '/texture_conversion_settings.json';
    if ((window as any).electronAPI.fsExistsSync(textureConversionSettingsPath)) {
