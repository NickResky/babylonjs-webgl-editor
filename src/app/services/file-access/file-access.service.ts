...
export class FileAccessService {
    ...
    public async getFileSize(filePath: string): Promise<number> {
        const stats = await (window as any).electronAPI.fsPromisesLstat(
            filePath
        );
        return stats.size;
    }
    ...
    public async addFile(
        ...
        const filePath =
            (window as any).electronAPI.path().join(basePath, fileName) +
            fileType;
        const exist = await this.exists(filePath);
    ...
    public async renameFile(
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            const newFilePath = (window as any).electronAPI
                .path()
                .join(basePath, newFileName + fileType);
            (window as any).electronAPI.fsRename(
                filePath,
                newFilePath,
                (err) => {
                    if (err) reject(err);
    ...
    public async copyFileFromPathToNewPath(
        filePath: string,
        newFilePath: string
    ): Promise<void> {
        const exist = await this.exists(filePath);
        return new Promise<void>((resolve, reject) => {
            ...
            (window as any).electronAPI.fsCopyFile(
                filePath,
                newFilePath,
                (err) => {
                    if (err) reject(err);
    ...
    public async copyFile(sourcePath: string, destinationPath: string) {
        ...
        await (window as any).electonAPI.fsPromisesMkdir(filePath, {
            recursive: true
        });
        return new Promise<string>((resolve, reject) => {
            (window as any).electronAPI.fsCopyFile(
                sourcePath,
                destinationPath,
                (error) => (error ? reject(error) : resolve(destinationPath))
            );
        });
    }
    ...
    public async getFilesInDirectory(directoryPath: string): Promise<string[]> {
        ...
        return new Promise<string[]>((resolve, reject) => {
            ...
            (window as any).electronAPI.fsReaddir(
                directoryPath,
                (err, files) => {
                    if (err) reject(err);
                    resolve(files);
