...
export class MaterialService {
  ...
  private _materialMappingsJSON$: BehaviorSubject<MVMaterialMappingsJson> = new BehaviorSubject({
    materialAllocators: [],
    switchMaterials: [],
  });
  public materialMappingsJSON$ = this._materialMappingsJSON$.asObservable();
  private _unmappedMaterials: string[] = [];
  private _unmappedMaterials$: BehaviorSubject<string[]> = new BehaviorSubject([]);
  ...
  public defaultMaterialsUrl$: BehaviorSubject<string> = new BehaviorSubject(null);
  ...
  public getMaterialMapping(basePath: string, fileName: string): Promise<MVMaterialMappingsJson> {
    const newPath = basePath.replace('file://', '');
    const newFileName = fileName.replace('.json', '');
    return new Promise((resolve, reject) => {
      this.fileService.getFile(newPath, newFileName, FileType.JSON).then((data: string) => {
        try {
  ...
  public safeNewMaterialMapping(basePath: string, fileName: string, data: string): Promise<void> {
    const newPath = basePath.replace('file://', '');
    const newFileName = fileName.replace('.json', '');
    return new Promise((resolve, reject) => {
      this.fileService.updateFile(newPath, newFileName, FileType.JSON, data).then(() => {
        this.notifier.notify('success', `Material Mapping ${fileName} saved`);
        resolve();
      });
    });
  }
  ...
  public createMaterial(
    ...
    return this.fileService.addFile(newPath, newFileName, FileType.JSON, JSON.stringify(data));
  }
  ...
  public updateMaterial(
    ...
    fileName = material['parentMaterialName'] ? material['parentMaterialName'] : fileName;
    const materialSerialized: any = material.serialize();
    if (material instanceof MVMaterial) {
      materialSerialized['indexOfRefraction'] = material.indexOfRefraction;
      if (material.detailMap.texture) {
        materialSerialized.detailMap = {
          texture: material.detailMap ? material.detailMap.texture.serialize() : null,
    ...
    if (materialClassName == 'NodeMaterial') {
      ...
      const pbrBlock: any = nodeMaterial.getBlockByName('PBRMetallicRoughness') as any;
      if (pbrBlock) {
        const serializedPbrBlock = materialSerialized.blocks.find(
          (b) => b.name == 'PBRMetallicRoughness'
        );
        serializedPbrBlock['environmentIntensity'] = pbrBlock.environmentIntensity;
        serializedPbrBlock['directIntensity'] = pbrBlock.directIntensity;
      }
    }
    const newPath = basePath.replace('file://', '');
    const newFileName = fileName.replace('.json', '');
  ...
  public async renameMaterial(
    ...
    const materialPath = url.slice(0, lastSlashIndex).replace('file://', '');
    ...
    entityUrls.forEach(async (entityUrl: string) => {
      ...
      if (entityConfig && lastSlashIndex) {
        ...
        const materialMappingsUrl = entityBaseUrl + entityConfig.materialMappingsUrlRelative;
  ...
  public getMaterial(basePath: string, fileName: string): Promise<any> {
    const newPath = basePath.replace('file://', '');
    return new Promise((resolve, reject) => {
      this.fileService.getFile(newPath, fileName, FileType.JSON).then((data: string) => {
        try {
  ...
  private async addAllMaterialsToScene(
    ...
    const arraysMatch = function (materials: string[], sceneMaterials: string[]) {
  ...
  public setMaterialMappingsJSON(materialMappingsJSON: MVMaterialMappingsJson) {
  ...
  public async onSaveAllocatorMapping(): Promise<void> {
    if (this.materialMappingForm.valid) {
      const copyMappingFile = JSON.parse(JSON.stringify(this._materialMappingsJSON));
      ...
      this._materialMappingsJSON$.next(this._materialMappingsJSON);
      await this.updateActiveMaterialMappingFile(this._materialMappingsJSON);
  ...
  private async updateActiveMaterialMappingFile(
    ...
    mappingFile.materialAllocators.sort((a: MVMaterialMappingJson, b: MVMaterialMappingJson) => {
      return a.name.toLowerCase().localeCompare(b.name.toLowerCase());
    });
    mappingFile.switchMaterials.sort((a: MVSwitchMaterialMapping, b: MVSwitchMaterialMapping) => {
      return a.name.toLowerCase().localeCompare(b.name.toLowerCase());
    });
  ...
  private async applyMaterialNameUpdateToMaterialMappingsFile(
    ...
    const lastSlashIndexOfMaterialMappingFileUrl = materialMappingFileUrl.lastIndexOf('/');
    ...
    const materialMappingFileName = materialMappingFileUrl
      ...
      .replace('.json', '');
    const materialMappingFile: MVMaterialMappingsJson = await loadJson(materialMappingFileUrl);

    if (!materialMappingFile.materialAllocators || !materialMappingFile.switchMaterials) {
      this.notifier.notify('error', `Unvalid file selected: ${materialMappingFileUrl}`);
      return;
    }

    // update material mappings
    materialMappingFile.materialAllocators.map((allocator) => {
      if (allocator.mapping === oldMaterialName) allocator.mapping = newMaterialName;
    });

    materialMappingFile.switchMaterials.map((switchMaterial) => {
      switchMaterial.slots.map((slot) => {
        if (slot.mapping === oldMaterialName) slot.mapping = newMaterialName;
      });
    });

    const activeMaterialMappingsUrl = this._activeEntity.entityConfig.materialMappingsUrlRelative;
    const isActiveMappingsFile = materialMappingFileUrl.includes(activeMaterialMappingsUrl);
  ...
  public async onSaveEditedAllocatorMapping(allocator: MVMaterialMappingJson): Promise<void> {
    const value = this.materialMappingForm.value;
    if (!value.mapping || value.mapping.length == 0) {
      return;
    }
    const materialMappingsCopy = JSON.parse(JSON.stringify(this._materialMappingsJSON));
    ...
    // update material mapping json file
    materialMapping.mapping = value.mapping ? value.mapping : allocator.mapping;
  ...
  public async deleteAllocator(allocator: MVMaterialMappingJson) {
    const i = this._materialMappingsJSON.materialAllocators.findIndex(
      (el: MVMaterialMappingJson) => el.name === allocator.name && allocator.mapping
    );
  ...
  public async addNewMaterial(newMaterialName: string, isNodeMaterial) {
    ...
    await this.createMaterial(
      ...
      .then(() => {
        this.notifier.notify('success', `Material ${newMaterialName} saved`);
  ...
  openEditSwitchDialog(data: { switchMaterialName: string; slot: MVMaterialMappingJson }) {
    this.materialMappingForm = this.formBuilder.group({
      name: '',
      mapping: '',
    });
  ...
  public async onSaveEditedSwitchMapping(data: {
    ...
    const value = this.materialMappingForm.value;
    if (value.mapping && value.mapping.length > 0 && value.mapping !== data.slot.mapping) {
      const i = this._materialMappingsJSON.switchMaterials.findIndex(
        (el: MVSwitchMaterialMapping) => el.name === data.switchMaterialName
      );
      let currentSwitchMaterial = this._materialMappingsJSON.switchMaterials[i];
      const slot = currentSwitchMaterial.slots.find((slot) => slot.name == data.slot.name);
  ...
  public async syncMappingsWithCws() {
    ...
    cwsMaterials.forEach((cwsMaterial) => {
      const existingMapping = this._materialMappingsJSON.switchMaterials.find(
        (switchMaterial) => switchMaterial.name == cwsMaterial.switch_material_name
      );
      if (!existingMapping) {
        ...
        cwsMaterial.material_slots.forEach((slot) => {
          const existingSlot = existingMapping.slots.find((s) => s.name == slot.material_slot_name);
          if (!existingSlot) {
            existingMapping.slots.push({ name: slot.material_slot_name, mapping: null });
          }
        });
      }
    });
  ...
  public toggleLightmaps() {
    ...
    this.core.getScene().materials.forEach((material) => {
      if (material['BJSNODEMATERIALEDITOR']) {
        const vcaoIntensityBlock = (material as NodeMaterial).getBlockByName('VCAO Intensity');
        ...
        if (vcaoIntensityBlock) {
          ...
          (vcaoIntensityBlock as InputBlock).value = vacoValue == 0 ? material['vcaoIntensity'] : 0;
        }
      } else {
        if (material['lightmapTexture'] || material['lightmapTextureDeactivated']) {
          if (this._lightmapsActive) {
            material['lightmapTextureDeactivated'] = material['lightmapTexture'];
            material['lightmapTexture'] = null;
          } else {
            material['lightmapTexture'] = material['lightmapTextureDeactivated']
  ...
  public toggleVertexColorHighlight() {
    ...
    this.core.getScene().materials.forEach((material) => {
      if (material['BJSNODEMATERIALEDITOR']) {
        const nodeMaterial = material as NodeMaterial;
        material['vertexColorHightlightActive'] = !this._vertexColorOverwriteActive;
        const vertexColorSplitterBlock = nodeMaterial.getBlockByName('VertexColorSplitter');
        const fragmentOutputBlock = nodeMaterial.getBlockByName('FragmentOutput');
        const vcaoMultiplyBlock = nodeMaterial.getBlockByName('VCAO Multiply Final');

        if (vertexColorSplitterBlock && fragmentOutputBlock && vcaoMultiplyBlock) {
          if (this._vertexColorOverwriteActive) {
            vcaoMultiplyBlock.connectTo(fragmentOutputBlock, { input: 'rgb' });
          } else {
            vertexColorSplitterBlock.connectTo(fragmentOutputBlock, {
              input: 'rgb',
              output: 'rgb',
            });
          }
  ...
  public getParentAndAffectedMaterials(material: Material): {
    ...
    if (isLightmapMaterial) {
      parentMaterial = scene.materials.find((material) => material.name == parentMaterialName);
      const affectedChildMaterials: Material[] = scene.materials.filter(
        (material) => material['parentMaterialName'] == parentMaterialName
      );
  ...
  public removeTextureFromNodeMaterial(material: NodeMaterial, textureType: string) {
    const textureBlock = material.getBlockByName(textureType) as TextureBlock;
    ...
    if (textureType == 'Albedo Texture') {
      ...
      const perturbNormalBlock = material.getBlockByName('Perturb normal');
