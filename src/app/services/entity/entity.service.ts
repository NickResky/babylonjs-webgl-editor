...
export class EntityService {
    ...
    public async loadEntity(environmentId: string): Promise<MVEntity> {
        ...
        const ruleEngine = await this.fileAccessService.getFile(
            ...
            this._entityConfigFile.ruleEngineConfigUrlRelative.replace(
                '.json',
                ''
            ),
            FileType.JSON
        ...
        const combinedEntityConfigFile =
        ...
        const combinedEntityConfigString =
            'data:' + JSON.stringify(combinedEntityConfigFile);
    ...
    subscribeToCWSConfigurationUpdates(cwsWebsocketSession, cwsId) {
        setTimeout(async () => {
            const newOptions = await this.getCurrentCWSConfiguration(
                cwsWebsocketSession,
                cwsId
            );
    ...
    async updateConfiguration() {
        ...
        await this._core.Product.updateConfiguration(
            this._activeEntity.uuid,
            allConfigurationCodes
        );
    ...
    public async toggleConfigurationCodes(codeToToggle: string[]) {
        ...
        if (activeOption) {
            ...
            const nextOptionIndex =
                currentOptionIndex == codeToToggle.length - 1
                    ? 0
    ...
    public async updateEnvironmentConfigurationForProduct(
        ...
        if (this._activeEntity) {
            const environmentCodeIndex =
                this.activeEntityConfigurtionCodes?.indexOf(
                    previousEnvironmentId
                );
            if (environmentCodeIndex >= 0) {
                this.activeEntityConfigurtionCodes[environmentCodeIndex] =
                    nextEnvironmentId;
    ...
    public async updateLightmapRegistryJSON(): Promise<void> {
        ...
        if (this._entityConfigFile.lightmapTexturesUrlRelative) {
            const getFilesFromLightmapTextureFolder = async (
                JSONUrl: string
            ): Promise<string[]> => {
                return await this.fileAccessService.getFilesInDirectory(
                    JSONUrl
                );
            };

            let fileListInFolder: string[] =
                await getFilesFromLightmapTextureFolder(registryJSONUrl);
            const lightmapRegistryExists: boolean =
                fileListInFolder.findIndex(
                    (fileName) =>
                        fileName ===
                        this.lightmapRegistryJSONFileName + FileType.JSON
                ) > -1;

            fileListInFolder = fileListInFolder.filter((e) =>
                e.includes(FileType.JPG)
            );
            ...
            if (lightmapRegistryExists) {
                ...
                fileListInFolder.length > 0 &&
                    this.updateLightmapRegistryJSON();
            }
        }
    }
    ...
    public async updateGlbFileRegistryJSON(
        ...
        if (relativeMeshesUrl) {
            let filesInMeshesDirectory: string[] =
                await this.fileAccessService.getFilesInDirectory(
                    absoluteMeshesUrl
                );

            const glbFileRegistryExists: boolean =
                filesInMeshesDirectory.includes(
     ...
     */
    public async updateEntityAnimationsConfig(
            ...
            .then(
                () => {
                    ...
                    this.notifierService.notify(
                        'success',
                        'The entity JSON has been saved successfully.'
                    );
                },
                () => {
                    console.log('reject');
                    this.notifierService.notify(
                        'error',
                        'Error while saving the entity JSON.'
                    );
                }
            );
    }
    ...
    public removeEntity(entity: MVEntity): void {
        const removeIndex = this._entities.findIndex(
    ...
    public async getAllEntityUrlsInBaseDirectory(
        ...
        const filesInBaseDirectory =
            await this.fileAccessService.getFilesInDirectory(entityBaseUrl);
     ...
     */
    public async isMeshFilePartOfActiveEntity(
        meshFileName: string
    ): Promise<boolean> {
        const fullPath =
            this._entityBaseUrl.replace('file://', '') +
        ...
        const filesInMeshDirectory: string[] =
            await this.fileAccessService.getFilesInDirectory(fullPath);
