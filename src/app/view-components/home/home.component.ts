...
import { BaseResolver, Core, MVEntity, MVEntityConfig } from '../../../mv-webgl-core';
...
export class HomeComponent implements AfterViewInit {
  @ViewChild('canvascontainer') private canvasContainer?: ElementRef<HTMLElement>;
  @ViewChild('canvascontainerwrapper') private canvasContainerWrapper?: ElementRef<HTMLElement>;
  ...
  ngAfterViewInit(): void {
    ...
    this.dataService.projectSettings$.subscribe((project: ProjectSettings) => {
      ...
      this.entityUrl = this.entityBaseUrl.replace('file://', '') + this.entityConfigFileName;
  ...
  private async setup() {
    ...
    // await this.actionItemService.setupActionItems(this.core, this._projectSettings);
    ...
    const environmentEntityConfigUrl = this.entityConfigFile.environmentEntityUrlsRelative[0];
    const environmentEntityConfig: MVEntityConfig =
      await this.environmentService.setEnvironmentEntityConfig(environmentEntityConfigUrl);

    this.scene = this.core.getScene();


    const loadEntityPromise = this.entityService.loadEntity(environmentEntityConfig.id);

    const loadEnvironmentPromise = this.environmentService.loadDefaultEnvironment(this.entityBaseUrl, this.entityConfigFile);
    const results = await Promise.all([loadEntityPromise, loadEnvironmentPromise]);
    ...
    // this.inspectorService.openInspector(this.scene, this.canvasContainerWrapper?.nativeElement);
  ...
  private async setupCamera() {
    return this.cameraService.setupCamera(this.core, this.entityBaseUrl, this.entityConfigFile, this.entityUrl);
  }
